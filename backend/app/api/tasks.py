...
async def _is_agent_available(
    ...
    *,
    ...
    """Check if an agent is still available on the board."""
    agent = await Agent.objects.by_id(agent_id).first(session)
    if agent is None:
        return False
    # Agent is available if it's on this board and not deleted/deleted
    if agent.board_id != board_id:
        return False
    if agent.status in ("deleted", "error"):
        return False
    return True
...
async def _reconcile_dependents_for_dependency_toggle(
    ...
    *,
    ...
    # Check board configuration for preserving assignment
    board = await Board.objects.by_id(board_id).first(session)
    preserve_assignment = (
        board.preserve_assignment_on_dependency_change if board else True
    )
    ...
    for dependent in dependents:
        ...
        if reopened:
            ...
            if should_reset:
                dependent.status = "inbox"
                # Only unassign if preserve_assignment is False OR agent is not available
                if dependent.assigned_agent_id is not None:
                    if not preserve_assignment or not await _is_agent_available(
                        session,
                        board_id=board_id,
                        agent_id=dependent.assigned_agent_id,
                    ):
