...
OPENAPI_TAGS = [
    ...
    },
    {
        ...
        "description": "Gateway webhook callbacks for real-time agent lifecycle event notifications.",
    },
    {
        "name": "metrics",
...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    # Start the heartbeat service for syncing agent statuses
    from app.services.openclaw.session_heartbeat_service import start_heartbeat_service, stop_heartbeat_service
    from app.db.session import async_session_maker
    
    heartbeat_service = None
    auto_promote_task = None
    auto_promote_running = False
    agent_logs_task = None
    ...
    # Start the task auto-promote background loop (runs every hour)
    try:
        from app.services.task_auto_promote import run_auto_promote_cycle
        auto_promote_running = True
        
        async def auto_promote_loop():
            while auto_promote_running:
                try:
                    async with async_session_maker() as session:
                        stats = await run_auto_promote_cycle(session)
                        if stats["promoted"] > 0:
                            logger.info("task_auto_promote.cycle_complete promoted=%d skipped=%d", 
                                       stats["promoted"], stats["skipped"])
                except Exception as exc:
                    logger.error("task_auto_promote.cycle_error error=%s", str(exc))
                await asyncio.sleep(3600)  # Run every hour
        
        auto_promote_task = asyncio.create_task(auto_promote_loop())
        logger.info("app.lifecycle.task_auto_promote_service.started")
    except Exception as exc:
        logger.warning("app.lifecycle.task_auto_promote_service.start_failed error=%s", str(exc))
    
    # Start the agent logs fetching service (runs every 30 seconds)
    try:
        from app.services.agent_logs import AgentLogsService
        agent_logs_running = True
        
        async def agent_logs_loop():
            while agent_logs_running:
                try:
                    async with async_session_maker() as session:
        ...
        logger.info("app.lifecycle.agent_logs_service.started")
    except Exception as exc:
        logger.warning("app.lifecycle.agent_logs_service.start_failed error=%s", str(exc))
    
    logger.info("app.lifecycle.started")
    try:
        yield
    finally:
        if auto_promote_running and auto_promote_task:
            auto_promote_running = False
            auto_promote_task.cancel()
            try:
                await auto_promote_task
            except asyncio.CancelledError:
                pass
            logger.info("app.lifecycle.task_auto_promote_service.stopped")
        
        if agent_logs_running and agent_logs_task:
            agent_logs_running = False
            agent_logs_task.cancel()
            try:
                await agent_logs_task
            except asyncio.CancelledError:
                pass
            logger.info("app.lifecycle.agent_logs_service.stopped")
        
        if heartbeat_service:
            try:
                await stop_heartbeat_service()
                logger.info("app.lifecycle.heartbeat_service.stopped")
            except Exception as exc:
                logger.warning("app.lifecycle.heartbeat_service.stop_error error=%s", str(exc))
...
api_v1.include_router(gateway_callbacks_router)
